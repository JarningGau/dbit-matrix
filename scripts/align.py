#!/usr/bin/env python3
"""Align demux/spike-in FASTQ pairs for DBiT workflow."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run bwa mem on demux FASTQ pairs and pipe to "
    ...
    parser.add_argument(
        "--bwa-index",
        required=True,
        help="bwa index prefix passed to bwa mem.",
    )
    parser.add_argument(
        "--bwa-threads",
        type=int,
        default=2,
        help="Thread count passed to bwa mem -t. Default: 2.",
    )
    parser.add_argument(
        "--bwa-bin",
        default="bwa",
...
def main() -> int:
    ...
    print(f"[align] job_count={len(jobs)}")
    print(f"[align] spike_in_count={len(spike_indexes)}")
    ...
    for index, (chunk, r1, r2) in enumerate(jobs, start=1):
        ...
        print(f"[align] ({index}/{len(jobs)}) chunk={chunk}")

        # Always align spike-ins first, then host genome.
        for spike_name, spike_index in spike_indexes:
            ...
            spike_command = f"{quoted(spike_bwa_cmd)} | {quoted(spike_samtools_cmd)}"
            print(f"[align] spike_in={spike_name}")
            print(f"[align] output_bam={spike_out_bam}")
            print(f"[align] command={spike_command}")
            if not args.dry_run:
                run_pipeline(spike_bwa_cmd, spike_samtools_cmd)
        ...
        host_command = f"{quoted(host_bwa_cmd)} | {quoted(sinto_cmd)}"
        print("[align] target=host")
        print(f"[align] output_bam={host_out_bam}")
        print(f"[align] command={host_command}")
        if not args.dry_run:
            run_pipeline(host_bwa_cmd, sinto_cmd)
