...
### mbias / call / saturation / summary / aggregate / methscan_prepare（实验）
...
- `methscan_run_script`：可选，覆盖默认 `scripts/methscan_run.py`（`methscan_prepare` 等 methscan 阶段均通过该脚本的不同子命令运行）
- `methscan_pixi_manifest`：可选，含 `pixi.toml` 的目录，传给 `scripts/methscan_run.py` 的 `--pixi-manifest`（默认仓库内 `envs/methscan`）
...
- Slurm 按 stage 配置：`slurm.split.split_bams`、`slurm.mbias.host`、`slurm.call.host`、`slurm.saturation`、`slurm.summary`；实验性 `aggregate`：`slurm.aggregate`、`aggregate_script`；实验性 `methscan_prepare`：`slurm.methscan_prepare`、`methscan_run_script`（可选）、`methscan_prepare_chunksize`（可选）
