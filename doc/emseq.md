...
- **可选 `methscan_prepare`（实验性）**：不纳入 `--stage all`；显式 `--stage methscan_prepare` 复用 `scripts/methscan_prepare.py`（pixi `envs/methscan`），从 `coverage/host/**/*.CG.cov` 写出 `coverage/host_prepare/`（契约见 `doc/stages.md` §12）
