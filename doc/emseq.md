...
- **methscan_prepare**（实验性）：`methscan_run_script`（默认 `scripts/methscan_run.py`）、`methscan_pixi_manifest`（可选，覆盖默认 `envs/methscan`）、`methscan_prepare_chunksize`（可选，默认 `10000000`）；Slurm：`slurm.methscan_prepare`
...
单 stage：`--stage` 改为 `fastp_split`、`demux_extract_bc`、`align`、`pool`、`split`、`mbias`、`call`、`saturation`、`summary`，或实验性 `aggregate` / `methscan_prepare`；Slurm 时 `--runner local` 改为 `--runner slurm`。
