#!/usr/bin/env python3
"""Extract DBiT barcodes from EMSeq paired FASTQ and write demultiplexed reads."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Locate linker2/Tn5 on R1, extract barcodeA/barcodeB from the "
            "EMSeq layout, filter by whitelist, and output paired FASTQ."
        )
    )
    ...
    parser.add_argument(
        "--linker2",
        default="ATTTATGTGTTTGAGAGGTTAGAGTATTTG",
        help="Expected linker2 sequence.",
    )
    parser.add_argument(
        "--linker1",
        default="TAAGTGTTGGTTTTTTGTATTT",
        help="Expected linker1 sequence.",
    )
    parser.add_argument(
        "--tn5",
        default="TATTGGTGTATGATTAGATGTGTATAAGAGATAG",
...
def main() -> int:
    ...
    prefix_len = len(linker1) + bc2_len + len(linker2) + bc1_len + OTHERS_LENGTH + len(tn5)
    t0 = time.monotonic()
    t_last = t0
    reads_last = 0
    ...
    def report_progress(force: bool = False) -> None:
        ...
        print(
            f"[extract_bc_emseq] progress reads={total} kept={kept} "
    ...
    with open_text(args.r1, "r") as r1_in, open_text(args.r2, "r") as r2_in:
        with gzip.open(
            ...
            for rec1, rec2 in zip_longest(fastq_iter(r1_in), fastq_iter(r2_in)):
                ...
                linker2_window_end = min(
                    len(s1_u), len(linker1) + bc2_len + len(linker2) + 12
                )
                ...
                linker1_start = bc2_start - len(linker1)
                bc1_end = linker2_end + bc1_len
                if linker1_start < 0 or bc1_end > len(s1_u):
                    reject_counts["structure_mismatch"] += 1
                ...
                linker1_obs = s1_u[linker1_start:bc2_start]
                ...
                tn5_pos = find_tn5(
                    s1_u,
                    tn5,
                    after_pos=bc1_end + OTHERS_LENGTH,
                    max_edit_distance=args.linker_edit_distance,
                )
                if tn5_pos is None:
                    reject_counts["structure_mismatch"] += 1
    ...
    print(f"[extract_bc_emseq] input_r1={args.r1}")
    print(f"[extract_bc_emseq] input_r2={args.r2}")
    print(f"[extract_bc_emseq] output_r1_demux={out_r1_demux}")
    print(f"[extract_bc_emseq] output_r1_spike_in={out_r1_spike}")
    print(f"[extract_bc_emseq] output_r2_demux={out_r2_demux}")
    print(f"[extract_bc_emseq] output_r2_spike_in={out_r2_spike}")
    print(f"[extract_bc_emseq] stats={out_stats}")
    elapsed = max(time.monotonic() - t0, 1e-9)
    keep_rate = (kept / total) if total else 0.0
    print(
        f"[extract_bc_emseq] kept={kept}/{total} keep_rate={keep_rate:.4f} "
