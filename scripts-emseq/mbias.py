#!/usr/bin/env python3
"""Run M-bias QC for host and spike-in BAMs (EMSeq / bisulfite-style reads).

EMSeq uses `biscuit align` on converted libraries. At reference CpG sites we
follow an asTair-like TOP/BOT interpretation instead of TAPS chemistry:

- TOP alignments (`99`, `147`): evaluate the reference `C` in `CG`, then treat
  read `C` as methylated and `T` as unmethylated.
- BOT alignments (`83`, `163`): evaluate the reference `G` paired with the
  preceding `C` in `CG`, then treat read `G` as methylated and `A` as
  unmethylated.

Context is currently restricted to `CG`. Outputs match the shared
`qc/mbias/` layout used by the TAPS pipeline so downstream paths stay stable.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(EMSeq bisulfite-style interpretation). "
...
def run_command(command: list[str], dry_run: bool) -> None:
    print(f"[emseq.mbias] command={quoted(command)}")
    if not dry_run:
...
def run_or_skip(command: list[str], output_path: Path, dry_run: bool) -> bool:
    if output_path.exists():
        print(f"[emseq.mbias] skip_existing_output={output_path}")
        return False
...
def classify_emseq_cpg(
    base: str,
    *,
    orientation: str,
) -> str | None:
    """Return 'methylated', 'unmethylated', or None if not classified."""
    b = base.upper()
    if orientation == "TOP":
        if b == "C":
            return "methylated"
...
def resolve_cpg_orientation(
    flag: int,
    ref_pos: int,
    reference_sequence: str,
) -> str | None:
    """Return TOP/BOT when the aligned reference base belongs to a CpG site."""
    if flag in TOP_FLAGS:
        ...
        if reference_sequence[ref_pos : ref_pos + 2] == "CG":
            return "TOP"
        return None
    if flag in BOT_FLAGS:
        if ref_pos - 1 < 0 or ref_pos >= len(reference_sequence):
            return None
        if reference_sequence[ref_pos - 1 : ref_pos + 1] == "CG":
            return "BOT"
        return None
    return None
        ...
        for record in bam_file.fetch(until_eof=True):
            ...
            for query_pos, ref_pos in record.get_aligned_pairs(matches_only=False):
                if query_pos is None or ref_pos is None:
                    continue
                orientation = resolve_cpg_orientation(
                    record.flag,
                    ref_pos,
                    current_sequence,
                )
                ...
                if cycle < 1 or cycle > max_cycle:
                    continue
                label = classify_emseq_cpg(read_seq[query_pos], orientation=orientation)
                if label is None:
                    continue
...
def run_host_mode(args: argparse.Namespace, work_path: Path, output_dir: Path) -> None:
    ...
    for command, out_path in build_prepare_host_subsample_commands(
        ...
        run_or_skip(command, out_path, args.dry_run)

    if host_tsv.exists() and host_png.exists():
        print(f"[emseq.mbias] skip_existing_output={host_tsv}")
        print(f"[emseq.mbias] skip_existing_output={host_png}")
    else:
        rows: list[dict[str, object]] = []
        host_sorted = host_paths.sorted_bam
        print(f"[emseq.mbias] host_input={host_sorted}")
        print(f"[emseq.mbias] host_output={host_tsv}")
        print(f"[emseq.mbias] host_plot={host_png}")
        if not args.dry_run:
        ...
        if not host_png.exists() and not args.dry_run:
            write_mbias_png(host_png, rows, "M-bias: host (EMSeq)", args.max_cycle)
...
def run_spike_mode(args: argparse.Namespace, work_path: Path, output_dir: Path) -> None:
    ...
    print(f"[emseq.mbias] spike_in_count={len(spike_names)}")
    for spike_name in spike_names:
        ...
        if output_path.exists() and plot_path.exists():
            print(f"[emseq.mbias] skip_existing_output={output_path}")
            print(f"[emseq.mbias] skip_existing_output={plot_path}")
            continue
        rows: list[dict[str, object]] = []
        print(f"[emseq.mbias] spike_in={spike_name}")
        print(f"[emseq.mbias] spike_input={spike_bam}")
        print(f"[emseq.mbias] output={output_path}")
        print(f"[emseq.mbias] plot={plot_path}")
        if not args.dry_run:
            rows = count_mbias_rows(
                spike_bam,
        ...
        if not plot_path.exists() and not args.dry_run:
            write_mbias_png(
                plot_path, rows, f"M-bias: {spike_name} (EMSeq)", args.max_cycle
            )
...
def main() -> int:
    ...
    print(f"[emseq.mbias] mode={args.mode}")
    print(f"[emseq.mbias] work_path={work_path}")
    print(f"[emseq.mbias] output_dir={output_dir}")
    print(f"[emseq.mbias] host_subsample_fraction={args.host_subsample_fraction}")
    print(f"[emseq.mbias] host_subsample_seed={HOST_SUBSAMPLE_SEED}")
    print(f"[emseq.mbias] host_subsample_max_reads={HOST_SUBSAMPLE_MAX_READS}")
    print(f"[emseq.mbias] max_cycle={args.max_cycle}")
