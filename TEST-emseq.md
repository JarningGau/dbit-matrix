...
- Stages：`fastp_split`、`demux_extract_bc`、`align`、`pool`、`split`、`mbias`、`call`、`saturation`、`summary`，实验性 `aggregate` / `methscan_prepare`（不纳入 `--stage all`），以及 `all`（生成 `commands/run.sh` / `run.sbatch` 串联全流程）
- 单步脚本：`scripts/fastp_split.py`、`scripts/extract_bc.py`、`scripts-emseq/aligner.py`、`scripts/pool.py`、`scripts/split_bams.py`、`scripts/bam_sort_parallel.py`、`scripts-emseq/mbias.py`、`scripts-emseq/call.py`、`scripts/saturation.py`、`scripts/summary.py`、`scripts/aggregate.py`（`aggregate`）、`scripts/methscan_prepare.py`（`methscan_prepare`）
...
1. CLI `--help` 可用
2. 主线九个 stage 与实验性 `aggregate` / `methscan_prepare` 在 `local` 与 `slurm` 下均能 dry-run；`--stage all` 在 `local` 与 `slurm` 下 dry-run（不落盘；`all` 不含 `aggregate` 或 `methscan_prepare`）
...
### Dry-run：`local`（主线九个 stage + `aggregate` + `methscan_prepare` + `all`）

```bash
CFG=workflow/dbit_emseq_test.json
STAGES="fastp_split demux_extract_bc align pool split mbias call saturation summary aggregate methscan_prepare"
...
### Dry-run：`slurm`（主线九个 stage + `aggregate` + `methscan_prepare` + `all`）
...
- `--stage all` 的顺序不变，且不包含 `aggregate` 或 `methscan_prepare`
